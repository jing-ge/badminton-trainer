...
- **开发改动**：
  - `app/(tabs)/index.tsx`：
    - 最近训练卡片改造为 Pressable，增加按压反馈与向右箭头，跳转至日志详情页。
    - 在同日非首条的最近训练列表顶端（recent[0]）新增小字体的与上次时长的对比数据行，颜色语义区分多/少/持平。
    - 快捷入口在有历史训练记录时，新增「训练记录」卡片，并应用了适当的间距和 emoji。
  - `package.json` & `app.json`：提升版本号至 v0.37.0。
- **路径修正记录**：原 PRD 跳转目标 `/training/log/[id]` 与项目现状不符（详情页实际在 `app/log/[id].tsx`，v0.28 创建），第一版交付落到打卡录入页，已由 orchestrator 修正：最近训练 → `/log/[id]`，「训练记录」快捷入口 → `/(tabs)/stats`。
- **测试结论**：
  - ✅ 最近训练卡片增加交互反馈并正确导向 `/log/[id]`。
  - ✅ 卡片最右侧增加了统一的提示角标 `›`，样式规范。
  - ✅ recent.length >= 2 时首条与第二条日期不同时显示与上次时长的对比并颜色编码。
  - ✅ 0 训练数据时不显示对比，也没有展示「训练记录」快捷入口，卡片依然表现正常。
  - ✅ package/app 版本更新及全局 typecheck 通过。
- **typecheck**：✅ `tsc --noEmit` 通过
