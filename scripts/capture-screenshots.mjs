...
const ROUTES = [
  ['home',           '/(tabs)'],
  ['train-weekly',   '/(tabs)/train'],
  ['stats-period',   '/(tabs)/stats'],
...
const chrome = spawn(CHROME, [
  ...
  '--user-data-dir=/tmp/chrome-cap-profile-v2',
  '--no-first-run',
...
await send('Runtime.evaluate', {
  expression: `
    (() => {
      const keys = ['prefs.onboardingDone', '@prefs.onboardingDone'];
      for (const k of keys) try { localStorage.setItem(k, '1'); } catch {}
