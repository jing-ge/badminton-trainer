...
// [name, path, scrollY]
const STEPS = [
  ['stats-history',    '/(tabs)/stats',     1400],
  ['tutorial-detail',  '/tutorial/clear',   0],
  ['run-idle',         '/training/run',     0],
];
...
const chrome = spawn(CHROME, [
  ...
  '--user-data-dir=/tmp/chrome-cap-profile-v3',
  '--no-first-run',
...
await send('Runtime.evaluate', {
  expression: `(() => { for (const k of ['prefs.onboardingDone','@prefs.onboardingDone']) try { localStorage.setItem(k,'1'); } catch{} })()`,
  returnByValue: true,
});

for (const [name, path, scrollY] of STEPS) {
  await navigate(APP_URL + path);
  if (scrollY) {
    await send('Runtime.evaluate', {
      expression: `
        (() => {
          // find first ScrollView (RN web compiles to div with overflow auto)
